...
function shuffle(state) {
  for (var a = 0; a < state.questions.length; a++) {
    var tempA = state.questions[a];
    var tempB = Math.floor(Math.random() * state.questions.length);
    state.questions[a] = state.questions[tempB];
    state.questions[tempB] = tempA;

    tempA = state.quizOptions[a];
    state.quizOptions[a] = state.quizOptions[tempB];
    state.quizOptions[tempB] = tempA;
  }  
  return state
}

function extraSlice(state) {
  state.questions = state.questions.slice(3)
  state.quizOptions = state.quizOptions.slice(3)
  return state;
}
...
class Quiz extends React.Component {
  state = {
    ...
    questions: [
      ...
      'Which is your favourite song, Peeka baby?', // Peeku Peeku Peeku Aaja Aaja Aaja, O Peeku Rani Oye hoye Oye hoye, Peeku Rani Peeku Rani Come Come Come, Jab tak rahega samose mein aalu, tak tak rahegi O meri Peeku 
      'Okay Jhingalala. Who is your favourite?', // Seeta-Geeta Laila Chhoti Peeku Potentially Chhoti Peeku
  ...
  render() {
    return (
      <div className="quiz">
        ...
        { this.state.result.length === 2 && <Link to={{
          pathname: '/result',
            state: {
